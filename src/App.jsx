...
function LinesDetail() {
  ...
  const [isSmall, setIsSmall] = useState(window.innerWidth <= 925);

  useEffect(() => {
    const handleResize = () => {
      setIsSmall(window.innerWidth <= 925);
    };
  ...
  // 여기서 if문으로 조건 분기 가능
  const isNarrow = isSmall; 
  const stationClass = isNarrow ? "station7" : "station8";
            ...
            {/* 역들(7자 이상 ... 붙임) */}
                <div className="linesdetail-stationscontainer">
                  <div className="linesdetail-stations linesdetail-hide-scrollbar" ref={listRef}>
                    ...
                    "station1",
                    idx === (isNarrow ? 6 : 7) && stationClass, // 조건으로 클래스 적용
                    ]
                    .join(" ");
                    return (
                         <div
                        ...
                        style={{
                        clipPath: isSmall
                          ? idx === 6 && "inset(0 0 50% 0)"
                          : idx === 7 && "inset(0 0 50% 0)",
                        overflow: "hidden",
                      }}
                          >
                        {displayName}
